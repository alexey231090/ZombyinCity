...
namespace EvolveGames
{
    ...
    public class PlayerController : MonoBehaviour
    {
        ...
        public static readonly Subject<bool> isGround = new();//Земля
        public static readonly Subject<bool> isRun = new();//Бег
        public static readonly Subject<bool> isDown = new();//Присел



        public Animator animatorHend;
        ...
        void Update()
        {
            ...
            if (Input.GetButtonDown("Jump") && canMove && characterController.isGrounded && !isClimbing)
            {
            ...
            /*Прятать оружие при подходе к стене, Заменить FireWepon.HideWepon на имя булевой переменной для аниматора
            if(WallDistance != Physics.Raycast(GetComponentInChildren<Camera>().transform.position, transform.TransformDirection(Vector3.forward), out ObjectCheck, HideDistance, LayerMaskInt) && CanHideDistanceWall)
            {
                ...
                animatorHend.SetBool(FireWepon.HideWepon, WallDistance);
        ...
        private void OnTriggerEnter(Collider other)
        {
            ...
            { 
               ...
               // Items.Hide(true);
                ...
                isGround.OnNext(false);//Земля
            }


            if (other.tag == "Ground")
            {
                isGround.OnNext(true);//Земля
